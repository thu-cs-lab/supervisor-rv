...
除此之外，默认情况下还会按照 [UART 16550 的规范](https://www.lammertbies.nl/comm/info/serial-uart) 进行一些寄存器的配置。在 QEMU 中运行的时候，请保持 `EN_UART16550=y`，这也是默认行为。如果你采用了自定义的实现，请设置 `EN_UART16550=n` 以去掉这些寄存器操作，或者忽略掉这些寄存器的操作（但初始化时仍然会输出额外的字符，因为 RBR THR 和 DLL 在同一个地址）。如果使用了 AXI UART16550 作为串口控制器，请参考代码注释并修改 `kernel/include/serial.h` 中的常量。
